...
# Model Evaluation Page
    ...
    # Store evaluation data in session state
    st.session_state['model_evaluation'] = {
        'model': model,
        'X_test': X_test,
        'y_test': y_test,
...
# Simulator Page
    ...
    **Simulate and Predict Fraudulent Transactions:**
    ...
    # Check if the model is available in the session state
    if 'model_evaluation' in st.session_state and 'model' in st.session_state['model_evaluation']:
        ...
        if st.button("Simulate"):
            ...
            try:
                ...
                # If 'feature_names_in_' is not available, use the columns from the training data
                if hasattr(model_sim, 'get_params'):
                    params = model_sim.get_params()
                    if 'feature_names_in' in params:
                        input_data = input_data[params['feature_names_in']]
                    else:
                        st.error("Model's expected features do not match the input features.")
                        st.stop()
                else:
                    st.error("Model's expected features do not match the input features.")
                    st.stop()
        ...
        st.error("No model found. Please perform model evaluation first.")
